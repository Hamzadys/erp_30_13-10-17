...
function get_open_balance($debtorno, $to, $convert)
{
    ...
    if ($convert)
    	$sql .= " * rate";
    $sql .= ", 
    	((t.ov_amount + t.ov_gst - t.ov_freight + t.ov_freight_tax + t.ov_discount - t.discount1 - t.discount2) * -1)";
    if ($convert)
    	$sql .= " * rate";
    $sql .= ")) AS OutStanding
...
$destination = $_POST['PARAM_9'];
	...
	$cols = array(0, 50, 120, 160,  200, 240, 280, 335, 400, 460, 530, 600);

	//$headers = array(_('No.'), _('Date/Narration'), _(''), _('Rate'), _('Qty'), _('Disc/Frt'),
	//	_('Total'), 	_('Dr'), _('Cr'), _('Balance'));

	
	$headers = array(_('No.'), _('Date/Narration'), _(''), _(''), _(''), _(''),
		_(''), 	_(''), _(''), _('Amount'));
    ...
    $rep = new FrontReport(_('Customer Receipt Report'), "CustomerReceipts", user_pagesize());
    $rep->Font();
...
//		if (!$convert && $currency != $myrow['curr_code']) continue;
		...
		$rep->fontSize -= 2;
		$rep->TextCol(3, 5,	_(""));
		//$rep->AmountCol(7, 8, $init[0], 0);
		//$rep->AmountCol(8, 9, $init[1], 0);
		//$rep->AmountCol(9, 10, $init[2], 0);
		$total = array(0,0,0,0);
	/*	for ($i = 0; $i < 4; $i++)
		{
			$total[$i] += $init[$i];
		...
		while ($trans = db_fetch($res))
		{
			...
			if(!$summary)
		   {
				...
				$rep->Font('bold');	
				$rep->AmountCol(5, 6, $trans['ov_freight'], 0);						
				$rep->AmountCol(7, 8, $item[0], 0);
				$rep->Font();				
		   }			
			...
			if(!$summary)
		   {
				...
				{
					$rep->Font(italic);
					$rep->TextCol(2, 8, $memo, -2);
					$rep->Font();
				}
				else
				$rep->NewLine();
	
				if ($trans['ov_discount'] != 0 )
				{
				$rep->TextCol(2, 4, _('GST WH / WHT') , -2);
				$rep->Font(italic);
				//$rep->AmountCol(4, 5, $trans['gst_wh'], $dec);
				$rep->AmountCol(5, 6, $trans['ov_discount'], $dec);
				$rep->AmountCol(6, 7, $trans['ov_amount'], $dec);
				$rep->Font();
				}
			...
			if(!$summary)
		   {
				$rep->AmountCol(9, 10, $item[1], 0); // shariq
		...
		// For loop for adding running balance column
			$rep->Font('bold');	
			for ($i = 2; $i < 3; $i++)
		//	$rep->AmountCol(9 ,10, $total[$i], 0); //Balance shariq cmnt
			$rep->Font();						
	...
	//	$rep->AmountCol($i + 3, $i + 4, $deb_sum_total[$i], $dec);			
		...
		for ($i = 0; $i < 3; $i++)
			...
			$rep->AmountCol(9,10, $total[1], 0); // sahriq
	...
	//	$rep->AmountCol($i + 3, $i + 4, $deb_sum_total_grandtotal[$i], $dec);
		//$rep->AmountCol($i + 7, $i + 8, $grandtotal[$i], 0);
		
		$rep->AmountCol(9, 10, $grandtotal[1], 0); //shariq
