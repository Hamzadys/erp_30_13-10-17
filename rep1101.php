...
function print_deliveries()
{
	...
	$cols = array(4, 60, 200, 300, 350, 425, 450, 515);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'left', 'left', 'left', 'right', 'right');
	$headers1 = array(_("Item Code"), _("Item Description"), _("Quantity"),
		_("Unit"), _("Inventory Location"), _(""), _("Total"));


//	$headers1 = array(_('Date'), _('Voucher No'), _('Description'), _('Chq No'),_('Debit'), _('Credit'), _('Balance'));
	...
	for ($i = $from; $i <= $to; $i++)
	{
			...
			$rep->SetCommonData($myrow, $branch, $sales_order, '', 133, $contacts);
			$rep->SetHeaderType('Header1101');
			$rep->NewPage();  
			...
			while ($myrow2=db_fetch($result))
			{
				...
				if ($Net != 0.0  || !is_service($myrow2['mb_flag']) || !$SysPrefs->no_zero_lines_amount())
				{
					...
					if ($packing_slip == 0)
					{
						$rep->TextCol(4, 5, get_location_name($so['from_stk_loc']));
//						$rep->TextCol(4, 5,	$DisplayPrice, -2);
		...
		$rep->Font('bold');
		$rep->MultiCell(100, 5, "Received By" , 0, 'L', 0, 2, 420,770, true);
		$rep->MultiCell(100, 5, "__________________" , 0, 'L', 0, 2, 400,759, true);
