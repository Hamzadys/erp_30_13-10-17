...
function print_inventory_sales()
{
	...
	$cols = array(0, 300, 350, 400, 450, 510);

	$headers = array(_('Customer'), _('Qty'), _('Sales'), _('Cost'), _('Contribution'));
	if ($fromcust != '')
		$headers[2] = '';
    ...
    $rep = new FrontReport(_('Inventory Sales Report - Summary'), "InventorySalesReport", user_pagesize(), 9, $orientation);
   	if ($orientation == 'L')
	...
	while ($trans=db_fetch($res))
	{
		...
		/*		$rep->NewLine(2, 3);
				...
				$rep->AmountCol(3, 4, $total_qty, $dec);  //qty
				...
				$rep->NewLine();
               */
				$total = $total1 = $total2 = $total_qty = 0.0;
		...
		//	$rep->TextCol(1, 2, $trans['description'].($trans['inactive']==1 ? " ("._("Inactive").")" : ""), -1);
			$rep->TextCol(0, 1, $trans['debtor_name']);
			}
		//	else
		//	$rep->TextCol(1, 2, $trans['description'].($trans['inactive']==1 ? " ("._("Inactive").")" : ""), -1);
			
			$rep->AmountCol(1, 2, $trans['qty'], get_qty_dec($trans['stock_id']));
		
		$rep->AmountCol(2, 3, $trans['amt'], $dec);
		$rep->AmountCol(3, 4, $trans['cost'], $dec);
...
//	$rep->NewLine(2, 1);
	...
	$rep->AmountCol(1, 2, $grandtotal_qty, $dec);  // qty
